...
#include "P_Ground_Attack02.h"

#include"Application/GameObject/Entitys/Character/Player/player.h"

void P_Ground_Attack02::Init()
{
}

void P_Ground_Attack02::Update()
{
	...
	else if (m_cnt >= TIME)
	{
		...
		{
			m_manager.lock()->ChangeAction("P_Ground_Attack03");
		}
...
void P_Ground_Attack02::ActionStart()
{
	KdEffekseerManager::GetInstance().Play("hit.efkefc", Math::Vector3(m_pPlayer.lock()->GetPos() + m_pPlayer.lock()->GetMatrix().Backward() * 5.0f + Math::Vector3(0, 1, 0)), 2.0f, 120, false);
	m_cnt = 0.0;

	m_pPlayer.lock()->SetAnimation("Attack", false);
}

void P_Ground_Attack02::ActionReset()
{
}
